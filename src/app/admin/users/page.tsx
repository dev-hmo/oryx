...
function Modal({ open, onClose, title, children }: {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-16">
            ...
            <div className="relative w-full max-w-lg bg-[#0f1628] border border-white/10 rounded-2xl shadow-2xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input
                        className={`${inputCls} ${errors.username ? "border-red-500/50" : ""} disabled:opacity-50`}
                        value={form.username}
                ...
                <div>
                    ...
                    <input
                        className={`${inputCls} ${errors.displayName ? "border-red-500/50" : ""} disabled:opacity-50`}
            ...
            <div className="flex gap-3 pt-2">
                ...
                <button onClick={onCancel} className="px-5 py-2.5 rounded-xl border border-white/10 text-white/50 hover:text-white text-sm transition-colors">
                    Cancel
                </button>
            </div>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-white/40">{users?.length || 0} admin accounts</p>
                    </div>
                </div>
                {canManage && (
                    <button
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="bg-white/[0.03] border-b border-white/[0.06]">
                            ...
                            <th className="text-left px-4 py-3.5 text-xs font-semibold text-white/40 uppercase tracking-wider">Role</th>
                            <th className="text-left px-4 py-3.5 text-xs font-semibold text-white/40 uppercase tracking-wider hidden md:table-cell">Joined</th>
                            <th className="text-left px-4 py-3.5 text-xs font-semibold text-white/40 uppercase tracking-wider hidden lg:table-cell">Last Login</th>
                            <th className="text-left px-4 py-3.5 text-xs font-semibold text-white/40 uppercase tracking-wider">Status</th>
                    ...
                    <tbody className="divide-y divide-white/[0.04]">
                        {sorted.map((user: any) => {
                            ...
                            return (
                                <tr key={id} className={`hover:bg-white/[0.02] transition-colors ${!user.isActive ? "opacity-50" : ""}`}>
                                    <td className="px-5 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500/30 to-indigo-500/30 border border-white/10 flex items-center justify-center">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {isMe && (
                                                        <span className="text-[9px] bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded font-semibold">YOU</span>
                                                    )}
                                                </div>
                                                <p className="text-xs text-white/30 font-mono">@{user.username}</p>
                                            </div>
                                        </div>
                                    </td>
            ...
            <div className="space-y-4 pt-10 border-t border-white/[0.06]">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-sm font-bold text-white">Role-Based Permissions</h3>
                        <p className="text-[11px] text-white/30">Define what each role can do across the system</p>
                    </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {useRbacStore.getState().roles.map((r) => (
                        <div key={r.name} className="group relative rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5 hover:bg-white/[0.04] transition-all overflow-hidden">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                {canManage && (
                                    <button
                                        ...
                                        className="p-1.5 rounded-lg text-white/20 hover:text-blue-400 hover:bg-blue-400/10 transition-colors opacity-0 group-hover:opacity-100"
                                    >
                                        <Pencil className="w-3.5 h-3.5" />
                                    </button>
                                )}
                            </div>
                            <p className="text-[11px] text-white/50 mb-4 line-clamp-2 leading-relaxed">
                                {r.description || "No description provided."}
                            </p>
                            <div className="flex flex-wrap gap-1.5">
                                {r.permissions.map((p) => (
                                    <span key={p} className="text-[9px] font-medium bg-white/[0.05] text-white/60 px-2 py-0.5 rounded border border-white/[0.05]">
                                        {p}
                                    </span>
                                ))}
                            </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex gap-3 pt-2">
                            <button
                                ...
                                className="flex-1 flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold transition-colors"
                            >
                                <Save className="w-4 h-4" /> Save Permissions
                            </button>
                            <button onClick={() => { setModal(null); setEditTarget(null); }} className="px-5 py-2.5 rounded-xl border border-white/10 text-white/50 hover:text-white text-sm transition-colors">
                                Cancel
                            </button>
                        </div>
                    </div>
